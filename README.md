...
- 数据源：`site/data.json`
- 顶栏可选择 GitHub 分支并读取该分支的 `site/data.json`；`dev` 支持 CRUD，其他分支只读
- GitHub Actions 会生成公开的 repo-only 数据；`dev` push 后自动同步 `site/data.json`

PR 和 `dev` / `main` push 会自动检查 PowerShell、Shell、JavaScript、JSON、skill 清单和敏感文件排除规则；只有 `main` 自动发布 Dashboard。Windows 用户不需要本地运行 Bash 或 `jq` 来生成公开数据。
