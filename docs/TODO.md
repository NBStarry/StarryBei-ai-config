...
- **现状（2026-06-15 更新）**：已盘点 `longxiabei` Mac 的 `~/.claude/plugins/`，把全部 marketplace / plugin / skill 记入 `claude/configs/recommended-plugins.json`（7 个 marketplace、22 个插件，含各插件携带的 skill 名）。新增发现并补录：`pua-skills`（tanweai/pua）、`telegram`、`pua`，以及全部官方插件。
- **已完成**：
...
- **仍待办**：
  - [ ] 盘点 **hanzebei** Mac（当前离线）——它可能有 longxiabei 之外的源/插件，需上线后补录、与本清单合并。
