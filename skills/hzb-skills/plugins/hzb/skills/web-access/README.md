...
给 Claude Code / Codex 装上完整联网能力的 skill。

Claude Code / Codex 原本有基础联网或本地工具能力，但缺少统一调度策略和浏览器自动化能力。这个 skill 补上的是：**联网策略 + CDP 浏览器操作 + 站点经验积累**。
...
Codex 使用时由本配置仓库的 `install.sh` 链接到 `~/.codex/skills/web-access`，不要把运行命令写死到 Claude 路径。
...
```bash
WEB_ACCESS_SKILL_DIR="${WEB_ACCESS_SKILL_DIR:-$HOME/.codex/skills/web-access}"
[ -f "$WEB_ACCESS_SKILL_DIR/scripts/check-deps.sh" ] || WEB_ACCESS_SKILL_DIR="$HOME/.agents/skills/web-access"
[ -f "$WEB_ACCESS_SKILL_DIR/scripts/check-deps.sh" ] || WEB_ACCESS_SKILL_DIR="$HOME/.claude/skills/web-access"
bash "$WEB_ACCESS_SKILL_DIR/scripts/check-deps.sh"
```

在 Codex 沙箱中，如果检查输出 `chrome: local TCP blocked by sandbox`，说明需要允许本地 `127.0.0.1` 端口访问后重跑；这不是 Chrome 未开启。
